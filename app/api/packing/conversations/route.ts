...
import { deleteShopperConversation, listTravelConversations, renameTravelConversation } from "@/lib/data/shopper-conversations";
...
export async function GET() {
  try {
    ...
    console.error("Travel archive unavailable.", error);
    return Response.json({ error: "Your saved travel conversations could not be loaded." }, { status: 503 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    return Response.json({ title: await renameTravelConversation(userId, id, title) });
  } catch (error) {
